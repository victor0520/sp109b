...
enum { // token : 0-127 直接用該字母表達， 128 以後用代號。因為 ASCII Code 定義的範圍是 0~127 共 128 字元，所以自訂的 token number 必須從 128 開始編號
...
void next() // 詞彙解析 lexer (parsing token)
{
  char *pp;

  while (tk = *p) { //最外層的 while 大部分情況都不會是終止條件，主要是由 while 內部直接 return，因此 next() 的邏輯是處理完一個 token 後就會直接 return
    ++p;
    if (tk == '\n') { // 換行
      if (src) {
...
{
  int t, *d;
  //第一部分利用 recursive descent (parsing)，檢查 symbol table (semantic Analyzer)，直接產生 IR (Code Generator)
  ...
  while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method，第二部分，Operator precedence (parsing)，主要處理四則運算
    t = ty;
    if (tk == Assign) {
      next();
